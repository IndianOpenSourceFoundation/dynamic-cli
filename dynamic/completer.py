...
from .completions import COMPLETIONS_DYNAMIC, SUBCOMMANDS
from .completions_git import META_LOOKUP_GIT, META_LOOKUP_GIT_EXTRAS
from .utils import TextUtils


class CompleterDynamic(Completer):
    ...
    def arg_completions(self, words, word_before_cursor):
        ...
        """
        if 'dynamic' not in words:
            return []
        for subcommand, args_opts in COMPLETIONS_DYNAMIC.items():
            if subcommand in words:
                args = list(COMPLETIONS_DYNAMIC[subcommand]['args'].keys())
                if not args:
                    # Some commands don't have args, complete options instead.
                    args = list(COMPLETIONS_DYNAMIC[subcommand]['opts'].keys())
